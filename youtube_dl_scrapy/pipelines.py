...
from youtube_dl_scrapy.flvcopycat import process_flv
from youtube_dl_scrapy.settings import TEMP_PATH,OUTPUT_PATH
class MyFilePipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item,FileItem):
            ...
            if item['fileid'] is None and item['end'] is None and item['id'] is None:
    ...
    def m3u8_combine(self,item):
        ...
        src = self.root + "%03d" % int(id) + "_" + name + "." + "ts"
    ...
    def flv_combine(self,item):
        ...
        id = item['id']
        now_id = self.namedict.setdefault(name, 1)
        src = self.root + "%03d" % int(id) + "_" + name + "." + "flv"
        temp_path = self.temp + name + str(item['fileid'])
        with open(temp_path, 'wb') as f:
            f.write(item['content'])
        while True:

...
def download_urls(
    ...
    try:
        ...
        my_defined_urls.setdefault("error",[]).append(tr(get_filename(title)))
        return
    if json_output:
    ...
    if dry_run:
        ...
        # print('Real URLs:\n%s' % '\n'.join(urls))
        my_dict=my_defined_urls.setdefault(tr(get_filename(title)),{})
        for index, value in enumerate(urls,start=1):
            my_dict.setdefault(index,value)
        my_dict.setdefault("filetype",ext)
        return
...
def download_rtmp_url(
    ...
    if dry_run:
        my_dict=my_defined_urls.setdefault(tr(get_filename(title)),{})
        for index, value in enumerate([url],start=1):
            my_dict.setdefault(index,value)
        my_dict.setdefault("filetype",ext)
        # print_user_agent(faker=faker)
        # print('Real URL:\n%s\n' % [url])
...
def script_main(download, download_playlist, **kwargs):
    ...
    if "my_own_urls" in kwargs:
        URLs = kwargs["my_own_urls"]
    else:
        URLs = []
    if args.input_file:
    ...
    try:
        ...
        if "my_own_playlist" in kwargs:
            args.playlist=kwargs["my_own_playlist"]
        for item in URLs:
            item=[item]
            download_main(
